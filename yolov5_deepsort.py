...
class VideoTracker(object):
    ...
    def __enter__(self):
        if self.args.cam != -1:
            ...
            self.video_frams = int(self.vdo.get(cv2.CAP_PROP_FRAME_COUNT))
            assert self.vdo.isOpened()

        if self.args.save_path:
            os.makedirs(self.args.save_path, exist_ok=True)
            videoname = str(self.video_path.split('/')[-1].split('.')[0])
            

            # path of saved video and results
            self.save_video_path = os.path.join(self.args.save_path, videoname + "_results.avi")
            self.save_results_path = os.path.join(self.args.save_path, videoname + "_results.txt")
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            if not len(cls_ids):
                ...
                print("Time: {:.03f}s,   Fps: {:.03f}".format(end - all_time, idx_frame / (end - all_time)))
                continue
...
def parse_args():
    ...
    parser.add_argument('--weights', type=str, default='detector/yolov5/weights/yolov5s.pt', help='model.pt path')
